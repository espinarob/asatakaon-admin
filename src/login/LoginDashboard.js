	...
	submitLoginInput = ()=>{
		...
		if(this.state.usernameInput.length == 0 ){
			...
			this.props.doUseFirebaseObject
				...
				.once("value",snapshot=>{
					if(snapshot.exists()){
						const currentAdminValid = JSON.parse(JSON.stringify(snapshot.val()));
						if( String(this.state.usernameInput)!=String(currentAdminValid.username)
							|| String(this.state.passwordInput)!=String(currentAdminValid.password) 
							){
							setTimeout(()=>{
								this.props.setDisplayLoading(false);
							},Constants.LOG_DISPLAY_TIME);
							this.setState({loginMessageError:'Username or password input is incorrect'});
						}
						else{
							setTimeout(()=>{
								this.props.setDisplayLoading(false);
				...
				.catch((error)=>{
					this.setState({loginMessageError:'Error in connecting to the server'});
				});
		}
	}
